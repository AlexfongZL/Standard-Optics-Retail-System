...
class CustomerController extends Controller
{
    ...
    // function to delete customer and all of it's degree and remove it's id from all sales record
    // sales record retain!
    public function delete_customer(Request $request){
        ...
        try{
            $degree = Degrees::where('customers_id', $request->input('customers_id'))
                            ->delete();
            
            $sales = Sales::where('customers_id', $request->input('customers_id'))
                            ->update([
                                'customers_id' => null
                            ]);

            $customer = Customers::where('id', $request->input('customers_id'))
                            ->delete();

            return redirect()->route('customer.list')->with('success', 'Customer Deleted Successfully. However, all the sales generated by the customer still remains. Only his name were deleted from the sale.');

        }catch (\Exception $e) {
            // Return error response if deletion fails
            return back()->withErrors(['error' => 'Failed to Delete Customer. Please contact Admin. Error: ' . $e->getMessage()]);
        }
    }
}
